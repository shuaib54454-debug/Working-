...
import {
  initializeFirestore, getFirestore, collection, doc, setDoc, deleteDoc,
  onSnapshot, getDocFromServer, writeBatch, query, where, Unsubscribe
} from "firebase/firestore";
import {
  getAuth, onAuthStateChanged, signInWithEmailAndPassword,
  createUserWithEmailAndPassword, signOut as firebaseSignOut,
  sendPasswordResetEmail, updatePassword as firebaseUpdatePassword, User,
...
if (typeof window !== "undefined") {
  (async () => {
    try { await setPersistence(auth, browserLocalPersistence); }
      ...
      catch { await setPersistence(auth, inMemoryPersistence).catch(() => {}); } }
  })();
}
...
async function withDbRetry<T>(fn: () => Promise<T>, retries = 2, delayMs = 350): Promise<T> {
  ...
  while (true) {
    try { return await fn(); }
      ...
      if (isDbClosing && attempt < retries) { attempt++; await new Promise(resolve => setTimeout(resolve, delayMs)); continue; }
      throw err;
    }
  }
}
...
try { firestoreInstance = initializeFirestore(app, { experimentalForceLongPolling: true, experimentalAutoDetectLongPolling: true }, databaseId); }
catch { firestoreInstance = databaseId ? getFirestore(app, databaseId) : getFirestore(app); }
...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null) {
  ...
  console.error("Firestore Error:", errInfo.error, operationType, path); return errInfo;
}
...
export async function loginWithEmail(email: string, pass: string): Promise<User> {
  ...
  return withDbRetry(async () => { const cred = await signInWithEmailAndPassword(auth, email.trim(), pass); if ((cred.user.email || "").toLowerCase() !== OWNER_EMAIL) { await firebaseSignOut(auth).catch(() => {}); throw new Error("This application is restricted to the owner account."); } return cred.user; });
}
...
export function subscribeToAuth(callback: (user: User | AppUser | null) => void): Unsubscribe {
  ...
  const unsub = onAuthStateChanged(auth, async firebaseUser => { if (!firebaseUser) { callback(null); return; } if ((firebaseUser.email || "").toLowerCase() !== OWNER_EMAIL) { await firebaseSignOut(auth).catch(() => {}); callback(null); return; } callback(firebaseUser); });
...
export async function uploadWorkerDocument(candidateId: string, folder: WorkerStorageFolder, fileOrBlob: File | Blob, customFileName?: string): Promise<{ downloadUrl: string; storagePath: string; docId: string; fileName: string; sizeBytes: number; isPdf: boolean }> {
  const uid = auth.currentUser?.uid; if (!auth.currentUser || !uid) throw new Error("Authentication required");
  ...
  const docId = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`; const path = `workers/${cleanCandidateId}/${folder}/${docId}-${fileName}`;
  ...
  if (!isPdf) { const compressedDataUrl = await compressImage(fileOrBlob, { maxWidth: 1400, maxHeight: 1400, quality: 0.82, mimeType: "image/jpeg" }); uploadBlob = dataUrlToBlob(compressedDataUrl); }
  ...
  catch (error) { console.error("Worker document upload failed:", error instanceof Error ? error.message : String(error)); throw new Error("Unable to securely upload the document. Please check your connection and try again."); }
}
...
export async function autoMigrateExistingDataToOwner(ownerUid: string, currentCandidates: Candidate[], currentExpenses: GeneralExpense[], currentSettings: AgencySettings, currentActivities?: ActivityLogEntry[]): Promise<{ candidatesMigrated: number; expensesMigrated: number; activitiesMigrated: number; settingsMigrated: boolean }> {
  if (!auth.currentUser || auth.currentUser.uid !== ownerUid || (auth.currentUser.email || "").toLowerCase() !== OWNER_EMAIL) return { candidatesMigrated: 0, expensesMigrated: 0, activitiesMigrated: 0, settingsMigrated: false };
  const activities = currentActivities ?? (() => { try { const raw = readLegacySensitiveValue(STORAGE_KEYS.activities); return raw ? JSON.parse(raw) as ActivityLogEntry[] : []; } catch { return []; } })();
  const candidatesToMigrate = currentCandidates.filter(c => !c.ownerUid || c.ownerUid === ownerUid);
  const expensesToMigrate = currentExpenses.filter(e => !e.ownerUid || e.ownerUid === ownerUid);
  const activitiesToMigrate = activities.filter(a => !a.ownerUid || a.ownerUid === ownerUid).map(a => ({ ...a, ownerUid }));
  let candidatesMigrated = 0, expensesMigrated = 0, activitiesMigrated = 0, settingsMigrated = false;
  if (candidatesToMigrate.length) { await migrateCandidatesStrict(candidatesToMigrate, ownerUid); candidatesMigrated = candidatesToMigrate.length; }
  if (expensesToMigrate.length) { await migrateExpensesStrict(expensesToMigrate, ownerUid); expensesMigrated = expensesToMigrate.length; }
  if (activitiesToMigrate.length) { await migrateActivitiesStrict(activitiesToMigrate, ownerUid); activitiesMigrated = activitiesToMigrate.length; }
  if (currentSettings && Object.keys(currentSettings).length) { await withDbRetry(() => setDoc(doc(db, "settings", ownerUid), { ...currentSettings, ownerUid }, { merge: true })); settingsMigrated = true; }
