    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-5 bg-black/70 backdrop-blur-xs animate-in fade-in">
      <div className="bg-[#fdfcfb] w-full max-w-4xl rounded-3xl shadow-2xl overflow-hidden border border-stone-200 flex flex-col max-h-[92vh]">
        <div className="bg-[#172a46] text-white p-4 sm:p-6 flex items-center justify-between border-b border-stone-800">
          <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-2xl bg-[#c9a84c] text-[#172a46] flex items-center justify-center font-black shadow-md"><ShieldCheck className="w-6 h-6" /></div><div><h3 className="font-black text-base sm:text-lg">ماسح جواز السفر الذكي</h3><p className="text-xs text-stone-300">اكتشف الجواز وسجل البيانات الموجودة فيه تلقائياً</p></div></div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
          {errorMessage && !(analysis?.overallStatus === "VERIFIED" && analysis.mrz?.checksums.allValid) && <div className="p-3.5 bg-amber-50 border border-amber-200 rounded-2xl flex items-center gap-2.5 text-xs text-amber-900 animate-in fade-in"><AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" /><span>{errorMessage}</span></div>}
          ...
          {activeMode === "UPLOAD" && <div className="bg-white p-5 rounded-2xl border-2 border-dashed border-stone-300 text-center hover:border-[#172a46] transition-colors relative"><input type="file" accept="image/*" onChange={handleFileUpload} className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" /><div className="flex flex-col items-center justify-center space-y-2"><div className="w-12 h-12 rounded-2xl bg-amber-50 text-[#c9a84c] flex items-center justify-center"><Upload className="w-6 h-6" /></div><h4 className="font-black text-sm text-[#172a46]">انقر هنا أو اسحب صورة صفحة جواز السفر</h4><p className="text-xs text-stone-500 max-w-md">يدعم JPG و PNG و WebP. ارفع صورة واضحة لصفحة بيانات الجواز وسيتم استخراج البيانات تلقائياً.</p></div>{selectedImage && <div className="mt-4 pt-4 border-t border-stone-100 flex flex-col sm:flex-row items-center justify-center gap-3"><div className="flex items-center gap-3"><img src={selectedImage} alt="Passport Preview" className="h-20 w-auto rounded-lg border border-stone-200 object-cover shadow-xs" /><div className="text-right text-xs text-stone-600"><p className="font-bold text-[#172a46]">تم تحميل الصورة</p>{isProcessing ? <p className="text-amber-600 flex items-center gap-1 font-bold"><RefreshCw className="w-3.5 h-3.5 animate-spin text-amber-600" /> جاري التعرف على الجواز واستخراج البيانات...</p> : statusMessage && !errorMessage ? <p className="text-emerald-600 font-bold flex items-center gap-1"><CheckCircle2 className="w-3.5 h-3.5" /> {statusMessage}</p> : <p className="text-stone-500 font-bold flex items-center gap-1">تم تحميل الصورة — بانتظار نتيجة التحليل</p>}</div></div></div>}</div>}
          ...
          {analysis && <div className="space-y-5 animate-in fade-in"><div className={`p-4 sm:p-5 rounded-2xl border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 ${analysis.overallStatus === "VERIFIED" ? "bg-emerald-50/80 border-emerald-300 text-emerald-950" : analysis.overallStatus === "NEEDS_REVIEW" ? "bg-amber-50/80 border-amber-300 text-amber-950" : "bg-rose-50/80 border-rose-300 text-rose-950"}`}><div className="flex items-start gap-3"><div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 ${analysis.overallStatus === "VERIFIED" ? "bg-emerald-600 text-white" : analysis.overallStatus === "NEEDS_REVIEW" ? "bg-amber-600 text-white" : "bg-rose-600 text-white"}`}>{analysis.overallStatus === "VERIFIED" ? <CheckCircle2 className="w-6 h-6" /> : analysis.overallStatus === "NEEDS_REVIEW" ? <AlertTriangle className="w-6 h-6" /> : <XCircle className="w-6 h-6" />}</div><div><h4 className="font-black text-sm sm:text-base">{analysis.overallStatus === "VERIFIED" ? "جواز السفر سليم ومطابق للمعايير" : analysis.overallStatus === "NEEDS_REVIEW" ? "تنبيه: يتطلب مراجعة إدارية" : "جواز غير صالح أو منتهي الصلاحية"}</h4><p className="text-xs font-medium mt-0.5 opacity-90">{analysis.overallSummary}</p></div></div><div className="flex items-center gap-3 bg-white px-4 py-2 rounded-2xl border border-stone-200 shadow-xs self-end sm:self-auto"><div className="text-right"><span className="text-[10px] text-stone-500 block">درجة النزاهة والمطابقة</span><span className="text-base font-black text-[#172a46]">{analysis.mrz ? `${analysis.integrityScore}%` : "غير متاحة"}</span></div><div className={`w-3 h-3 rounded-full ${analysis.integrityScore >= 85 ? "bg-emerald-500" : analysis.integrityScore >= 50 ? "bg-amber-500" : "bg-rose-500"}`} /></div></div>
            ...
            <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden shadow-2xs"><div className="bg-stone-50 px-4 py-3 border-b border-stone-200 flex items-center justify-between"><h5 className="text-xs font-black text-[#172a46] flex items-center gap-2"><Eye className="w-4 h-4 text-[#c9a84c]" />جدول مطابقة الحقول المتقاطعة (VIZ vs MRZ Cross-Matching):</h5><span className="text-[11px] text-stone-500">{analysis.mrz ? `${analysis.crossChecks.filter((c) => c.status === "pass").length} من أصل ${analysis.crossChecks.length} حقول متطابقة` : "لا يمكن إجراء المطابقة قبل توفر MRZ صالح"}</span></div><div className="divide-y divide-stone-100">{analysis.crossChecks.map((check, idx) => <div key={idx} className="p-3 sm:px-4 sm:py-3.5 flex flex-col sm:flex-row sm:items-center justify-between gap-2 hover:bg-stone-50/60 transition-colors"><div className="flex items-center gap-3"><div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs shrink-0 ${check.status === "pass" ? "bg-emerald-100 text-emerald-700" : check.status === "warning" ? "bg-amber-100 text-amber-700" : "bg-rose-100 text-rose-700"}`}>{check.status === "pass" ? <Check className="w-3.5 h-3.5" /> : check.status === "warning" ? <AlertTriangle className="w-3.5 h-3.5" /> : <X className="w-3.5 h-3.5" />}</div><div><span className="font-bold text-xs text-[#172a46]">{check.fieldLabel}</span><p className="text-[11px] text-stone-500 mt-0.5">{check.message}</p></div></div><div className="flex items-center gap-4 text-xs font-mono self-end sm:self-auto"><div className="text-left bg-stone-50 px-2.5 py-1 rounded-lg border border-stone-200"><span className="text-[9px] text-stone-400 block">MRZ:</span><span className="font-bold text-emerald-700">{check.mrzValue || "-"}</span></div><span className="text-stone-300">↔</span><div className="text-left bg-stone-50 px-2.5 py-1 rounded-lg border border-stone-200"><span className="text-[9px] text-stone-400 block">البصري:</span><span className="font-bold text-[#172a46]">{check.visualValue || "-"}</span></div></div></div>)}</div></div>
        ...
        <div className="bg-stone-100 px-4 sm:px-6 py-4 border-t border-stone-200 flex flex-col gap-3">{errorMessage && !(analysis?.overallStatus === "VERIFIED" && analysis.mrz?.checksums.allValid) && <div className="w-full p-2.5 bg-amber-50 border border-amber-300 rounded-xl flex items-center gap-2 text-xs text-amber-900 animate-in fade-in"><AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" /><span className="font-bold">{errorMessage}</span></div>}<div className="flex flex-col sm:flex-row items-center justify-between gap-3 w-full"><div className="flex items-center gap-2 w-full sm:w-auto"><button type="button" onClick={handleCopyAuditReport} disabled={!analysis} className="bg-white hover:bg-stone-50 text-stone-700 border border-stone-200 px-4 py-2.5 rounded-2xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all disabled:opacity-50 cursor-pointer">{copiedReport ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}<span>{copiedReport ? "تم نسخ التقرير!" : "نسخ تقرير الفحص"}</span></button></div><div className="flex items-center gap-2.5 w-full sm:w-auto"><button type="button" onClick={onClose} className="w-1/2 sm:w-auto px-5 py-2.5 rounded-2xl font-bold text-xs text-stone-600 hover:bg-stone-200 transition-colors cursor-pointer">إلغاء</button><button type="button" onClick={handleApply} disabled={isProcessing || !analysis?.mrz?.checksums.allValid} title={!analysis?.mrz?.checksums.allValid ? "لا يمكن الاعتماد قبل التحقق الكامل من MRZ وأرقام التحقق وفق ICAO 9303." : "اعتماد وتعبئة بيانات المرشح"} className="w-1/2 sm:w-auto min-h-[44px] bg-[#172a46] hover:bg-[#223d64] text-white px-6 py-2.5 rounded-2xl font-black text-xs shadow-md flex items-center justify-center gap-2 transition-transform active:scale-95 cursor-pointer touch-manipulation disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-[#172a46]"><CheckCircle2 className="w-4 h-4 text-[#c9a84c]" /><span>اعتماد وتعبئة بيانات المرشح</span></button></div></div></div>
      </div>
    </div>
  );
};
