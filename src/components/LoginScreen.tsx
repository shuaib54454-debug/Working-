...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onSuccess, onContinueLocal }) => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err?.code === "auth/unauthorized-domain" || msg.includes("auth/unauthorized-domain")) {
        ...
        setError(
          isAr
            ? `النطاق الحالي (${window.location.hostname}) غير مضاف في النطاقات المصرح بها في Firebase. يمكنك الدخول فوراً عبر (الجلسة المحلية) أدناه.`
            : `Current domain (${window.location.hostname}) is not authorized in Firebase Console. You can enter instantly using Local Owner Session below.`
        );
      } else {
  ...
  const handleContinueAsLocalOwner = () => {
    const localOwner: AppUser = {
      uid: "owner-shuaib-local",
      email: OWNER_EMAIL,
      displayName: isAr ? "شعـيب (مالك الوكالة)" : "Shuayb (Owner)",
      isLocal: true,
    };
    setLocalUser(localOwner);
    if (onContinueLocal) {
      onContinueLocal(localOwner);
    }
    onSuccess();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (code === "auth/operation-not-allowed" || rawMsg.includes("auth/operation-not-allowed")) {
        ...
        setError(
          isAr
            ? "طريقة الدخول بكلمة المرور (Email/Password) غير مفعّلة في مشروع Firebase. يُنصح بالدخول بحساب Google المفعّل مباشرة، أو الدخول كمالك محلياً."
            : "Email/Password sign-in provider is not enabled in your Firebase Console. Please sign in with Google or use the direct Local Owner session."
        );
      } else if (
        ...
        setError(
          isAr
            ? "بيانات الدخول غير صحيحة، أو لم يتم إنشاء كلمة مرور لهذا الحساب بعد في Firebase. يمكنك الضغط على 'إنشاء كلمة مرور' أو الدخول عبر Google."
            : "Invalid credentials or password has not been created yet in Firebase. You can create a password or sign in with Google."
        );
      ...
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8 text-stone-800 border border-[#c9a84c]/20 relative">
          ...
          <div className="mb-5 p-4 bg-amber-50 border border-amber-200 rounded-2xl text-xs text-amber-900 space-y-3">
            <div className="flex items-start gap-2.5">
              ...
              <div>
                ...
                <p className="text-[11px] text-amber-700 mt-1 leading-relaxed">
                  {isAr
                    ? "لتفادي هذا الخطأ، يمكنك الدخول فوراً بضغطة واحدة باستخدام حساب Google المعتمد، أو الدخول كمالك محلياً:"
                    : "To resolve this instantly, use your verified Google Account or start a Local Owner Session:"}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1">
              <button
                ...
                className="w-full py-2.5 px-3 bg-white border border-stone-300 hover:bg-stone-50 text-stone-800 font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all"
              >
                ...
                {googleLoading ? (isAr ? "جاري الدخول..." : "Signing in...") : (isAr ? "دخول فوري بـ Google" : "Sign in with Google")}
              </button>

              <button
                type="button"
                onClick={handleContinueAsLocalOwner}
                className="w-full py-2.5 px-3 bg-[#172a46] hover:bg-[#203a60] text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all"
              >
                <Zap className="w-3.5 h-3.5 text-[#c9a84c] shrink-0" />
                {isAr ? "دخول المالك محلياً" : "Owner Local Session"}
              </button>
            </div>
            ...
            <div className="pt-2 border-t border-amber-200/60">
              ...
              >
                ...
                {isAr ? "كيفية تفعيل كلمة المرور من لوحة Firebase؟" : "How to enable Email/Password in Firebase?"}
              </button>

              {showConsoleGuide && (
                <div className="mt-2 p-2.5 bg-white/80 rounded-xl text-[11px] text-stone-700 leading-relaxed border border-amber-200">
                  <ol className="list-decimal list-inside space-y-1">
                    <li>
                      {isAr ? "افتح تبويب Firebase Console المفتوح في متصفحك أو اضغط:" : "Open the Firebase Console tab or click:"}{" "}
                      <a
                      ...
                      >
                        Firebase Sign-in Providers <ExternalLink className="w-2.5 h-2.5 inline" />
                      </a>
                    </li>
          ...
          <div className="mb-4 p-3.5 bg-rose-50 border border-rose-200 rounded-2xl text-xs text-rose-700 font-bold flex flex-col gap-2">
            ...
            {isUnauthorizedDomain && (
              <button
                type="button"
                onClick={handleContinueAsLocalOwner}
                className="mt-1 py-2 px-3 bg-[#172a46] text-white text-xs font-bold rounded-xl flex items-center justify-center gap-1.5"
              >
                <Zap className="w-3.5 h-3.5 text-[#c9a84c]" />
                {isAr ? "المتابعة كمالك التطبيق (جلسة محلية)" : "Continue as Owner (Local Session)"}
              </button>
            )}
          </div>
        )}
        ...
        {mode === "login" && (
          <div className="mb-5">
            <button
              ...
              className="w-full py-3.5 px-4 rounded-2xl bg-stone-50 hover:bg-stone-100 border border-stone-200 text-stone-800 font-bold text-sm flex items-center justify-center gap-3 shadow-sm transition-all disabled:opacity-50"
            >
              ...
              <span>
                {googleLoading
                  ...
                  : (isAr ? "تسجيل الدخول بحساب Google (المعتمد)" : "Sign in with Google (Owner Account)")}
              </span>
            </button>
            ...
            <div className="relative my-4 flex items-center justify-center">
              ...
              <span className="bg-white px-3 text-[11px] font-bold text-stone-400 uppercase tracking-wider shrink-0">
                {isAr ? "أو الدخول بكلمة المرور" : "Or with password"}
              </span>
              <div className="border-t border-stone-200 w-full" />
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading || googleLoading}
            className="w-full py-3.5 rounded-2xl bg-[#172a46] hover:bg-[#203a60] text-white font-black text-sm flex items-center justify-center gap-2 shadow-md transition-all disabled:opacity-50"
          >
            {mode === "login" ? (
            ...
            {loading
              ...
              : mode === "login"
              ? (isAr ? "دخول المالك" : "Owner Sign In")
              : mode === "register"
        ...
        {/* Offline / Emergency Local Access */}
        <div className="mt-6 pt-4 border-t border-stone-100 flex flex-col items-center">
          <button
            type="button"
            onClick={handleContinueAsLocalOwner}
            className="text-[11px] font-bold text-stone-500 hover:text-[#172a46] flex items-center gap-1.5 transition-colors"
          >
            <Zap className="w-3.5 h-3.5 text-[#c9a84c]" />
            <span>
              {isAr
                ? "الدخول المباشر لحساب المالك (جلسة محلية آمنة)"
                : "Direct Owner Access (Local Offline Session)"}
            </span>
          </button>
          <p className="mt-2 text-center text-[10px] text-stone-400">
            {isAr
              ? "مخصص حصرياً للمالك المصرح له (shuaib54454@gmail.com)."
              : "Exclusively restricted to the authorized owner (shuaib54454@gmail.com)."}
          </p>
        </div>
      </div>
    </div>
